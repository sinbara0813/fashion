...
public class ChangeLoginActivity extends BaseActivity {
    ...
    private void sendCode(final TextView codeTv) {
        ...
        D2CApplication.httpClient.loadingRequest(api, new BeanRequest.SuccessListener<BaseBean>() {
            ...
            public void onResponse(BaseBean baseBean) {
                Util.showToast(ChangeLoginActivity.this, R.string.msg_send_code_ok);
            }
        }, new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
                Util.showToast(ChangeLoginActivity.this, Util.checkErrorType(error));
            }
        });
    }
    ...
    private void save() {
        ...
        ResetPasswordApi api = new ResetPasswordApi();
        api.setCode(code);
        api.setInterPath(RESET_PASSWORD_URL);
        api.setLoginCode(bindedAccount);
        api.setPassword(password);
        api.setConfirmPassword(password);
        loadingDialog.show();
        D2CApplication.httpClient.loadingRequest(api, new BeanRequest.SuccessListener<BaseBean>() {
            @Override
            public void onResponse(BaseBean baseBean) {
                loadingDialog.dismiss();
                Util.showToast(ChangeLoginActivity.this, R.string.msg_change_password_ok);
                UserBean.DataEntity.MemberEntity userBean = Session.getInstance().getUser();
                if (userBean != null) {
                    PushManager.getInstance().unBindAlias(ChangeLoginActivity.this, String.valueOf(userBean.getMemberId()), true);
                }
                Session.getInstance().logout(ChangeLoginActivity.this);
                setResult(RESULT_OK);
                finish();
            }
        }, new Response.ErrorListener() {
