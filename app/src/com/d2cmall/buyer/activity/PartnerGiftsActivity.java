...
import com.d2cmall.buyer.adapter.PartnerGiftsAdapter;
...
//买手我的礼包界面
public class PartnerGiftsActivity extends BaseActivity {
    ...
    private List<PartnerGiftsBean.DataBean.GiftsBean.ListBean> giftsList;
    private int currentPage = 1;
    private boolean hasNext =true;
    private VirtualLayoutManager mLayoutManager;
    private DelegateAdapter delegateAdapter;
    private BaseVirtualAdapter<DefaultHolder> endAdapter; //列表结束标志
    private PartnerGiftsAdapter partnerGiftsAdapter;
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                switch (newState) {
                    case RecyclerView.SCROLL_STATE_IDLE:
                        ...
                        if (last > partnerGiftsAdapter.getItemCount() - 3 && hasNext) {
                            currentPage++;
                            loadGifts();
                        }
                }
    ...
    private void init() {
        ...
        partnerGiftsAdapter = new PartnerGiftsAdapter(this, giftsList);
        ...
        delegateAdapter.addAdapter(partnerGiftsAdapter);
    }

    private void loadGifts() {
        progressBar.setVisibility(View.VISIBLE);
        SimpleApi api = new SimpleApi();
        api.setInterPath(Constants.PARTNER_GIFTS);
        api.setP(currentPage);
        api.setPageSize(20);
        D2CApplication.httpClient.loadingRequest(api, new BeanRequest.SuccessListener<PartnerGiftsBean>() {
            @Override
            public void onResponse(PartnerGiftsBean partnerGiftsBean) {
                ...
                if(currentPage==1 && (partnerGiftsBean.getData().getGifts()==null || partnerGiftsBean.getData().getGifts().getList()==null || partnerGiftsBean.getData().getGifts().getList().size()==0)){
                    setEmptyView(Constants.NO_DATA);
                    return;
                }
                giftsList.addAll(partnerGiftsBean.getData().getGifts().getList()) ;
                if (giftsList != null) {
                    partnerGiftsAdapter.notifyDataSetChanged();
                }
                hasNext = partnerGiftsBean.getData().getGifts().isNext();
            }
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                Util.showToast(PartnerGiftsActivity.this,Util.checkErrorType(error));
            }
        });
    }
