...
public class GiftControl implements GiftFrameLayout.LeftGiftAnimationStatusListener {
     ...
     * 礼物队列
     */
    private ArrayList<LiveAnimationBean> mGiftQueue;
    ...
    public void loadGift(LiveAnimationBean gift) {
        loadGift(gift, true);
    }
     ...
     */
    public void loadGift(LiveAnimationBean gift, boolean supportCombo) {
        if (mGiftQueue != null) {
            if (supportCombo) {
                ...
                for (int i = 0; i < mGiftLayoutParent.getChildCount(); i++) {
                    ...
                    if (giftFrameLayout.isShowing()) {
                        if (giftFrameLayout.getCurrentGiftId().equals(gift.getContent()) && giftFrameLayout.getCurrentSendUserId().equals(gift.getNickname())) {
                            //连击
                            giftFrameLayout.setGiftCount(gift.count);
                            giftFrameLayout.setSendGiftTime(gift.time);
                            return;
                        }
                    }
                }
    ...
    private void addGiftQueue(final LiveAnimationBean gift, final boolean supportCombo) {
        if (mGiftQueue != null) {
        ...
        if (supportCombo) {
            ...
            for (LiveAnimationBean model : mGiftQueue) {
                if (model.getContent().equals(gift.getContent()) && model.getNickname().equals(gift.getNickname())) {
                    model.count=(model.count + gift.count);
                    addflag = true;
                    break;
                }
            }
...
//            giftFrameLayout.firstHideLayout();
     ...
     * @return
     */
    private synchronized LiveAnimationBean getGift() {
        LiveAnimationBean gift = null;
        if (mGiftQueue.size() != 0) {
...
//                Iterator<LiveAnimationBean> iterator = mGiftQueue.iterator();
