...
public class TagFlowLayout extends FlowLayout implements TagAdapter.OnDataChangedListener {
    ...
    private Set<Integer> mSelectedView = new LinkedHashSet<>();
    private List<Integer> mEnableView = new ArrayList<>();
    ...
    private void changeAdapter() {
        ...
        LinkedHashSet preCheckedList = mTagAdapter.getPreCheckedList();
    ...
    public boolean performClick() {
        ...
        if (mEnableView.contains((Integer) pos)) {
            return true;
        }
        if (child != null) {
            doSelect(child, pos);
    ...
    public void setEnablelist(List<Integer> enablelist) {
        this.mEnableView = enablelist;
    }

    public void setEnable(Integer index, boolean is) {
        if (is) {
            if (!mEnableView.contains(index))
                mEnableView.add(index);
        } else {
            if (mEnableView.contains(index))
                mEnableView.remove(index);
        }
    }
    ...
    public Set<Integer> getSelectedList() {
        return new LinkedHashSet<>(mSelectedView);
    }
    ...
    private void doSelect(TagView child, int position) {
        if (mAutoSelectEffect) {
            ...
            if (mOnSelectListener != null) {
                mOnSelectListener.onSelected(new LinkedHashSet<>(mSelectedView));
            }
        }
    }
