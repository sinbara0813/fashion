...
package com.d2cmall.buyer.widget.universalvideoview;
...
//    private boolean armeaEnabled = false;
    ...
    private GifImageView mImageBuy;
    ...
    private void init(Context context) {
        ...
        View viewRoot = inflater.inflate(R.layout.layout_player_controller, this);
    ...
    private void initControllerView(View v) {
        ...
        mImageBuy = (GifImageView) v.findViewById(R.id.img_buy);
        try {
            gifFromAssets = new GifDrawable(getResources(), R.mipmap.buy);
        } catch (IOException e) {
            e.printStackTrace();
        }
        mImageBuy.setImageDrawable(gifFromAssets);
    ...
    public void reset() {
        ...
        mEndTime.setText(s);
        remaindTime.setText(s);
        mProgress.setProgress(0);
        mTurnButton.setImageResource(R.mipmap.ic_live_play_bottom);
    ...
    private String stringForTime(int timeMs) {
        ...
        if (changePlayButton) {
            if (hours == 0) {
                return mFormatter.format("%2d:%02d", minutes, seconds).toString();
            } else {
                return mFormatter.format("%2d:%2d:%02d", hours, minutes, seconds).toString();
            }
        }
    ...
    public void setSeekBarStyle(Drawable progressDrawable, Drawable thumbDrawable) {
        if (mProgress != null) {
            Rect rect = mProgress.getProgressDrawable().getBounds();
            mProgress.setProgressDrawable(progressDrawable);
            ((SeekBar) mProgress).setThumb(thumbDrawable);
            mProgress.getProgressDrawable().setBounds(rect);
        }
    }
    ...
    public int setProgress() {
        ...
        if (mEndTime != null) {
            ...
            remaindTime.setText("-" + stringForTime(duration - position));
        }
    ...
    public void updatePausePlay() {
        if (mPlayer != null && mPlayer.isPlaying()) {
            if (changePlayButton) {
                mTurnButton.setImageResource(pauseId);
            } else {
                mTurnButton.setImageResource(R.mipmap.ic_live_pause_bottom);
            }

//            mCenterPlayButton.setVisibility(GONE);
        } else {
            if (changePlayButton) {
                mTurnButton.setImageResource(playId);
            } else {
                mTurnButton.setImageResource(R.mipmap.ic_live_play_bottom);
            }
//            mCenterPlayButton.setVisibility(VISIBLE);
     ...
     * 是否修改播放按钮图标
     */
    private boolean changePlayButton = false;
    private int playId, pauseId;

    public void updatePausePlay(int playId, int pauseId) {
...
//    }
    ...
    public void setBuyListener(OnClickListener listener) {
        mImageBuy.setOnClickListener(listener);
    }
    ...
    private SeekChangeBack seekChangeBack;

    public void setSeekChangeBack(SeekChangeBack back) {
        this.seekChangeBack = back;
    }

    public interface SeekChangeBack {
        void seekChangeStart();
