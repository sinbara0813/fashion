...
public class VideoExtractFrameAsyncUtils {
    ...
    public void getVideoThumbnailsInfoForEdit(String videoPath, String OutPutFileDirPath, long startPosition, long endPosition, int thumbnailsCount) {
        ...
        for (int i = 0; i < thumbnailsCount; i++) {
            ...
            if (i == thumbnailsCount - 1) {
                if (interval > 1000) {
                    String path = extractFrame(metadataRetriever, endPosition - 800, OutPutFileDirPath,thumbnailsCount);
                    sendAPic(path, endPosition - 800,OutPutFileDirPath);
                } else {
                    String path = extractFrame(metadataRetriever, endPosition, OutPutFileDirPath,thumbnailsCount);
                    sendAPic(path, endPosition,OutPutFileDirPath);
                }
            } else {
                String path = extractFrame(metadataRetriever, time, OutPutFileDirPath,thumbnailsCount);
                sendAPic(path, time,OutPutFileDirPath);
            }
        }
        metadataRetriever.release();
    }
    ...
    private void sendAPic(String path, long time,String outPutFilePath) {
        VideoEditInfo info = new VideoEditInfo();
        info.path = path;
        info.time = time;
        info.outPutPath=outPutFilePath;
        Message msg = mHandler.obtainMessage(ExtractFrameWorkThread.MSG_SAVE_SUCCESS);
        msg.obj = info;
        mHandler.sendMessage(msg);
    }
    ...
    private String extractFrame(MediaMetadataRetriever metadataRetriever, long time, String OutPutFileDirPath,int num) {
        File appDir = new File(OutPutFileDirPath);
        if (appDir.exists()) {
            int lenth=appDir.listFiles().length;
            if (lenth==num){
                return null;
            }
        }
        ...
        if (bitmap != null) {
            Bitmap bitmapNew= scaleImage(bitmap);
            String path= PictureUtils.saveImageToSDForEdit(bitmapNew, OutPutFileDirPath);
            if (bitmapNew!=null &&!bitmapNew.isRecycled()) {
                bitmapNew.recycle();
                bitmapNew = null;
            }
