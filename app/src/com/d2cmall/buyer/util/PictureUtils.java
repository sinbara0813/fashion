...
public class PictureUtils {
    ...
    private static final String EDIT_PATH = "/video/";
    public static final String SHORT_VIDEO="/shortVideo/";
    private static String EDIT_VIDEO_PATH="/editVideo/";
    ...
    public static String saveImageToSDForEdit(Bitmap bmp, String dirPath) {
        if (bmp == null) {
            return "";
        }
        File appDir = new File(dirPath);
        ...
        String fileName=(lenth+1)+POSTFIX;
        File file = new File(appDir, fileName);
        try {
            FileOutputStream fos = new FileOutputStream(file);
            bmp.compress(Bitmap.CompressFormat.JPEG, 80, fos);
    ...
    public static String getEditVideoDir(Context context){
        String state = Environment.getExternalStorageState();
        File rootDir = state.equals(Environment.MEDIA_MOUNTED) ? context.getExternalCacheDir() : context.getCacheDir();
        File folderDir = new File(rootDir.getAbsolutePath() + EDIT_VIDEO_PATH);
        if (!folderDir.exists() && folderDir.mkdirs()) {
